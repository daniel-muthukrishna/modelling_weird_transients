...
def read_synapps_solution(xStr, activeStr, vref=10.0):
    ...
    for i in range(numOfIons):
        ...
        for j in range(numOfOpacityParams):
            ...
            if param == 'log_tau':
                logtau = float(ionInfo[ionName]['log_tau'])
            if param == 'aux':
                aux = float(ionInfo[ionName]['aux'])
            if param == 'v_min':
                vmin = float(ionInfo[ionName]['v_min'])
            if param == 'v_max':
...
def print_and_save_solutions(saveFilename, xStr, activeStr, vref=10):
    setupInfo, ionInfo = read_synapps_solution(xStr, activeStr, vref)
    print(setupInfo)
    print(ionInfo)
